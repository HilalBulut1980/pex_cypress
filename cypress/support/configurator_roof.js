...
module.exports = {
    ...
    configureRoof: function configureRoof(test) {
        ...
        it(test.name, function () {
            ...
            let df_aufschlag = test.df_aufschlag;
            let df_aufschlag_mwst = (jsonLogic.apply({ '*': [jsonLogic.apply({ '/': [df_aufschlag, 119] }), test.vat] })).toFixed(2);
            let df_aufschlag_2 = test.df_aufschlag_b;
            let df_aufschlag_mwst_2 = (jsonLogic.apply({ '*': [jsonLogic.apply({ '/': [df_aufschlag_2, 119] }), test.vat] })).toFixed(2);

            // GRUNDPREISE
            // 1. Stoff:
            let grundpreis = test.grundpreis;
            let grundpreis_df = (jsonLogic.apply({ '+': [grundpreis, df_aufschlag] })).toFixed(2);
            let grundPreis_red = (jsonLogic.apply({ '-': [grundpreis_df, test.discount_2] })).toFixed(2);  //Abzug 1. Rabatt
            ...
            let grundPreis_red_2 = (jsonLogic.apply({ '-': [grundpreis_df_2, test.discount_2b] })).toFixed(2);  //Abzug 1. Rabatt
            ...
            setModell(test.df_switcher, test.df_hersteller, test.df_produkt, test.df_typ)
            setMaße(test.df_switcher, test.df_glasbreite, test.df_glashoehe, test.df_falztiefe, test.df_fluegelbreite, test.df_fluegelhoehe, test.df_falzart)
...
function setMaße(switcher, breite, hoehe, tiefe, finnenbreite, finnenhoehe, falz) {
    if (switcher == "Ungenormt") {
        cy.get('#glasbreite').type(breite, { force: true })
        cy.get('#glashoehe').type(hoehe, { force: true })
        cy.get('#glasleistentiefe').type(tiefe, { force: true })
        cy.get('#fluegelinnenmass').type(finnenbreite, { force: true })
        cy.get('#fluegelhoehe').type(finnenhoehe, { force: true })
        cy.contains(new RegExp("^" + falz + "\\s*$")).click({ force: true })
    }
}
...
function setModell(switcher, hersteller, produkt, typ) {
    if (switcher == "Genormt") {
        cy.get('select[name="df_hersteller"]').select(hersteller, { force: true })
        cy.get('select[name="df_product"]').select(produkt)
        cy.get('select[name="df_product_type"]').select(typ)
    }
}

function setSchienenfarbe(schienenfarbe) {
    cy.get('input[value=' + schienenfarbe + ']').check({ force: true })
}
